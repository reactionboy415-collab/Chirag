import webbrowser
webbrowser.open("https://t.me/+6i-uLZtpcpczMjQ1")
import requests
import random
import json, string
...
print(f""" -  DEC BY RIZZ PY """) 

print("  ┌──────────────────────────────────────────┐                ")
# This will try to get the values from Render. 
# If you run it locally, it will use the default values you type inside the quotes.
token = os.getenv('BOT_TOKEN', '8289108955:AAF_Aey68df5qj5zoSQo3LvEHOzwPjAB04Q')
ID = os.getenv('7840042951')
...
def pppp():
    os.system('clear')
    output = (f"{green}••••••••••••••••••••••••••••••••\n"
              f"{reset}║ {red} 𝐇𝐢𝐭𝐬: {red}{hits} "
              f"{white} 𝐁𝐚𝐝 𝐢𝐧𝐬𝐭𝐚: {red}{badinsta} "
              f"{white} 𝐁𝐚𝐝 𝐞𝐦𝐚𝐢𝐥: {green}{bademail} "
              f"{white} 𝐆𝐨𝐨𝐝 𝐈𝐆: {blue}{goodig} {cyan}║\n"
...
def gg():
    while True:
        data = {
            ...
            "variables": json.dumps({"id": int(random.randrange(5699785217,  8597939245)), "render_surface": "PROFILE"}),
